...
def upload_name_for_script(message):
    ...
    bot.send_message(message.chat.id, "Отправь мне описание скрипта", reply_markup=gen_cancel_button())
...
def find_script(message):
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("MultiLine", callback_data="s4"))
...
def select_script_for_view(call):
    ...
    bot.send_message(call.message.chat.id,
                     f"<strong>{name.capitalize()}</strong>\nОт @{author.capitalize()}\nОписание: {description.capitalize()}\n\n<pre><code>"
                     f"{script_text}</code></pre>", parse_mode="html")
